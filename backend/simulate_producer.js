...
function getRandomOdometer(currentOdometer = 0) {
...
function getRandomVIN() {
    ...
    return Array.from(17, () => VIN_chars.charAt(Math.floor(Math.random() * VIN_chars.length))).join('');
}
...
function getRandomTelematics() {
    const timestamp = getRandomTimestamp();
    const vin = getRandomVIN();
    const odometer = getRandomOdometer();
